...
mod tests {
    ...
    fn rejects_position_high_bits_above_depth_two_tx_block() {
        // 2-tx block: root = sha256d(tx0 || tx1). Positions differing only in
        // bits above the single path level (2, 4, 6, ...) reconstruct the same
        // root because the surplus bits are never consulted. Reject them so
        // `tx_position` is authenticated by the proof.
        let tx0: Sha256d = [0x11; 32];
        let tx1: Sha256d = [0x22; 32];
        let root = dsha256_pair(&tx0, &tx1);
    ...
    fn rejects_position_high_bits_above_depth_four_tx_block() {
        ...
        // Position 6 (= 2 | 0b100) shares the low bits but sets a third,
    ...
    fn empty_path_rejects_nonzero_position() {
        // Single-tx block: the only valid position is 0.
        let txid: Sha256d = [0x42; 32];
        let root = txid;
        assert!(verify_merkle_proof(&txid, 0, &[], &root).is_ok());
        assert!(matches!(
